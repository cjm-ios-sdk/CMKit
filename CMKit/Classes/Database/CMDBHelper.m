...
+ (BOOL)createTableIfNotExists:(NSString *_Nonnull)tableName
    ...
    if (!result) {
        NSLog(@"CMDBUsers warning: Can't create table:%@", tableName);
    }
...
+ (BOOL)updateTable:(NSString *_Nonnull)tableName
    ...
    for (NSString *tmpKey in values.allKeys) {
        ...
        sql = [sql stringByAppendingFormat:@"%@=%@,", tmpKey, CMDBValue(tmpValue)];
    }
...
+ (BOOL)updateTable:(NSString *_Nonnull)tableName
    ...
    BOOL result = [db executeUpdate:sql];
    if (!result) {
        NSLog(@"CMDBUsers warning: Can't add primary key for table:%@", tableName);
    }
    return result;
}
