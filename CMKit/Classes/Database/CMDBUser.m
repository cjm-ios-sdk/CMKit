...
#define CMDBDefaultName @"default"

#define CMDBVersionTableName @"version"

#define CMDBVersionTableFields  @{\
...
- (instancetype _Nonnull)initWithDbName:(NSString *_Nullable)dbName mainDirectory:(NSString * _Nullable)mainDirectory {
    ...
    if (self) {
        ...
        _dbName = dbName ? [dbName copy] : CMDBDefaultName;
...
- (FMDatabaseQueue *)queue {
    ...
    [_queue inDatabase:^(FMDatabase *db) {
        BOOL flag = NO;
        flag = [CMDBHelper createTableIfNotExists:CMDBVersionTableName withTableFields:CMDBVersionTableFields inDB:db];
        if (!flag) {
            NSLog(@"创建version表失败。");
        }
    }];
...
- (void)cjm_checkUpgradeWithTableName:(NSString *)tableName withTableFields:(NSDictionary *)tableFields {
    NSString *condition = [NSString stringWithFormat:@"table_name=%@", CMDBValue(tableName)];
    NSArray *tableInfos = [self selectAllFromTable:CMDBVersionTableName
    ...
    if (tableInfos.count > 0) {
        ...
        if (result == NSOrderedAscending) {
            [self.queue inTransaction:^(FMDatabase * _Nonnull db, BOOL * _Nonnull rollback) {
                ...
                NSString *condition = [NSString stringWithFormat:@"table_name=%@", CMDBValue(tableName)];
                [CMDBHelper updateTable:CMDBVersionTableName withValues:info condition:condition inDB:db];
            }];
        }
    }
}
...
- (BOOL)createTableAndInsertRecordIfNotExists:(NSString *_Nonnull)tableName withTableFields:(NSDictionary *_Nonnull)tableFields {
    ...
    if ([tableName isEqualToString:CMDBVersionTableName]) {
        return NO;
    }
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        flag = [CMDBHelper createTableIfNotExists:tableName withTableFields:tableFields inDB:db];
        if (!flag) {
    ...
    NSString *condition = [NSString stringWithFormat:@"table_name=%@", CMDBValue(tableName)];
    NSInteger count = [self selectCountFromTable:CMDBVersionTableName withCondition:condition];
    ...
    if (0 == count) { 
        ...
        BOOL insertFlag = [self insertIntoTable:CMDBVersionTableName withValues:values lastRowId:nil];
        if (insertFlag) {
...
- (BOOL)dropTableAndDeleteRecordIfExists:(NSString *_Nonnull)tableName {
    ...
    __block BOOL flag = NO;
    [self.queue inDatabase:^(FMDatabase *db) {
        flag = [CMDBHelper dropTableIfExists:tableName inDB:db];
        
        NSString *condition = [NSString stringWithFormat:@"table_name=%@", CMDBValue(tableName)];
        if ([CMDBHelper selectCountFromTable:CMDBVersionTableName withCondition:condition inDB:db] > 0) {
            flag = [CMDBHelper deleteFromTable:CMDBVersionTableName withCondition:condition inDB:db];
        }
    }];
...
- (BOOL)updateTable:(NSString *_Nonnull)tableName addPimaryKeys:(NSArray *_Nonnull)primaryKeys {
    ...
    __block BOOL flag = NO;
    [self.queue inDatabase:^(FMDatabase *db) {
        flag = [CMDBHelper updateTable:tableName addPimaryKeys:primaryKeys inDB:db];
    }];
    return flag;
}
...
- (BOOL)deleteFromTable:(NSString *_Nonnull)tableName withCondition:(NSString *_Nullable)condition {
    ...
    [self.queue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        @try {
            flag = [CMDBHelper deleteFromTable:tableName withCondition:condition inDB:db];
...
- (BOOL)isExistTable:(NSString *_Nonnull)tableName {
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        isExist = [CMDBHelper isExistTable:tableName inDB:db];
    }];
...
- (NSInteger)selectCountFromTable:(NSString *_Nonnull)tableName withCondition:(NSString *_Nullable)condition {
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        count = [CMDBHelper selectCountFromTable:tableName withCondition:condition inDB:db];
    }];
...
- (NSArray *_Nonnull)selectAllFromTable:(NSString *_Nonnull)tableName
    ...
    [self.queue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        @try {
            array = [CMDBHelper selectAllFromTable:tableName
                                      withCondition:condition
                                              order:order
                                              limit:limit
                                               inDB:db];
...
- (BOOL)insertIntoTable:(NSString *_Nonnull)tableName
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        @try {
            result = [CMDBHelper insertIntoTable:tableName withValues:values lastRowId:lastRowId inDB:db];
            if (lastRowId) {
...
- (BOOL)replaceIntoTable:(NSString *_Nonnull)tableName
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        @try {
            result = [CMDBHelper replaceIntoTable:tableName withValues:values lastRowId:lastRowId inDB:db];
            if (lastRowId) {
                *lastRowId = [db lastInsertRowId];
            }
        } @catch (NSException *exception) {
...
- (BOOL)updateTable:(NSString *_Nonnull)tableName
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        @try {
            result = [CMDBHelper updateTable:tableName withValues:values condition:condition inDB:db];
        } @catch (NSException *exception) {
            NSLog(@"%s exception=%@", __func__, exception.description);
        }
    }];
